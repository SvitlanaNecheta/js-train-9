...
function findElementGreaterThan(arr, num) {
  ...
  else{// повертаємо undefined
  const found=arr.find(el=>el>num);
  return found;}// Використовуємо метод find для пошуку першого елементу, який є більшим за задане число num.
...
function findIndexByProperty(arr, name) {
 ...
 else{const foundedInd=arr.findIndex(el=>el.name==name);
 return foundedInd;
...
function findLastEvenNumber(arr) {
  ...
  else{const founded=arr.findLast(el=>el%2===0);// За допомогою метода findLast знаходимо останнє парне число в масиві.
  return founded }// Повертаємо знайдене число або 'Парних чисел не знайдено', якщо таке число не знайдено.
}
...
function getSubArrayAndConvertToString(arr, startIdx) {
  if(!Array.isArray(arr)){return 'Вхідний параметр має бути масивом'} // Перевіряємо, чи вхідний параметр є масивом, якщо ні повертаємо 'Вхідний параметр має бути масивом'
  else{const arr1=arr.slice(startIdx,arr.length);// За допомогою методу slice() отримуємо підмасив від заданого індексу до кінця масиву.
  return arr1.join(` `);// За допомогою методу join() конвертуємо підмасив в рядок.
  }// Повертаємо рядок.
}
...
function getArrayKeys(arr) {
  if(!Array.isArray(arr)){return 'Вхідний параметр має бути масивом'}// Перевіряємо, чи вхідний параметр є масивом, якщо ні повертаємо 'Вхідний параметр має бути масивом'
  else{ let arrayKeys=arr.keys();// За допомогою методу keys() отримуємо об'єкт ітератора, який містить ключі масиву.
  arrayKeys=Array.from(arrayKeys);// Конвертуємо ітератор в масив за допомогою методу from.
...
function getPositiveArrayValues(arr) {
  if(!Array.isArray(arr)){return 'Вхідний параметр має бути масивом'}// Перевіряємо, чи вхідний параметр є масивом, якщо ні повертаємо 'Вхідний параметр має бути масивом'
  else{ let iterValue=Array.from(arr.values()); // За допомогою методу values() отримуємо об'єкт ітератора, який містить значення масиву.
  let positiveArrayValues=iterValue.filter(elem=>(elem>0));
...
function removeAndDouble(arr, startIdx) {
  if(!Array.isArray(arr)){return 'Вхідний параметр має бути масивом'}// Перевіряємо, чи вхідний параметр є масивом, якщо ні повертаємо 'Вхідний параметр має бути масивом'
  else{ let doubledArray=arr.splice(0,startIdx)// За допомогою методу splice() видаляємо елементи масиву починаючи з заданого індексу.
  doubledArray=doubledArray.map(el=>el*2);// За допомогою методу map() подвоюємо кожен елемент масиву.
  return doubledArray;
...
function sumAndPrint(arr) {
  if(!Array.isArray(arr)){return 'Вхідний параметр має бути масивом'} // Перевіряємо, чи вхідний параметр є масивом, якщо ні повертаємо 'Вхідний параметр має бути масивом'
  else{const sum=arr.reduce((accumulator,currentValue)=>accumulator+currentValue);// За допомогою методу reduce() обчислюємо суму елементів масиву.
...
function flattenSquareAndFilter(arr, threshold) {
  if(!Array.isArray(arr)){return 'Вхідний параметр має бути масивом'}// Перевіряємо, чи вхідний параметр є масивом, якщо ні повертаємо 'Вхідний параметр має бути масивом'
  else{let filteredSquaredArray=arr.flat();// За допомогою методу flat() вирівнюємо масив.
  filteredSquaredArray=filteredSquaredArray.map(el=>el*el);// За допомогою методу map() підносимо кожен елемент масиву до квадрату.
  filteredSquaredArray=filteredSquaredArray.filter(el=>el>threshold);// За допомогою методу filter() фільтруємо елементи, які більше за порогове значення.
...
function getLastElementsSumAndPrint(arr, n) {
  if(!Array.isArray(arr)){return 'Вхідний параметр має бути масивом'}// Перевіряємо, чи вхідний параметр є масивом, якщо ні повертаємо 'Вхідний параметр має бути масивом'
  else{arr=arr.slice(n+1,arr.length);
   // За допомогою методу slice() вибираємо останні n елементів масиву.
  let sum=arr.reduceRight((accumulator,currentValue)=>accumulator+currentValue);// За допомогою методу reduceRight() обчислюємо суму останніх n елементів масиву.
  arr.forEach(el=>console.log(el));// За допомогою методу forEach() виводимо кожен елемент з останніх n в консоль.
...
function wordLengths(arr) {
  if(!Array.isArray(arr)){return 'Вхідний параметр має бути масивом'}// Перевіряємо, чи вхідний параметр є масивом, якщо ні, повертаємо 'Вхідний параметр має бути масивом'
  else{let lengthsArray=arr.map((el)=>el.length);// Використовуємо метод map для створення нового масиву, де кожний елемент - це довжина відповідного слова з вхідного масиву
...
function insertElementAt(arr, index, element) {
  if(!Array.isArray(arr)){return 'Вхідний параметр має бути масивом'} // Перевіряємо, чи вхідний параметр є масивом, якщо ні повертаємо 'Вхідний параметр має бути масивом'
  else{arr.splice(index,0, element);// Використовуємо метод splice для вставки елемента на позицію index
...
function flattenArray(arr) {
  if(!Array.isArray(arr)){return 'Вхідний параметр має бути масивом'} // Перевіряємо, чи вхідний параметр є масивом, якщо ні повертаємо 'Вхідний параметр має бути масивом'
  else{let flattenedArray=arr.flat(Infinity);// Використовуємо метод flat з параметром Infinity, щоб "випрямити" масив на будь-яку глибину вкладеності
...
function expandArrayByN(arr, n) {
  if(!(Array.isArray(arr)&&(typeof(n)==="number"))){return 'Перший вхідний параметр має бути масивом, другий - числом'} // Перевіряємо, чи вхідні параметри є масивом та числом відповідно, якщо ні повертаємо 'Перший вхідний параметр має бути масивом, другий - числом'
  else{let expandedArray=Array(); 
    expandedArray.length=n*arr.length;
    let i=0;
    while(i<expandedArray.length)
    {expandedArray=arr.flatMap((el)=>[el,el,el]);
    i++;}
...
function findLongestWord(arr) {
  if(!Array.isArray(arr)){return []} // Перевіряємо, чи вхідний параметр є масивом, якщо ні, повертаємо пустий рядок
 else{ const longestWord=arr.reduce((el,max)=>max.size<el.size?max=el: max=max);// Використовуємо метод reduce() для знаходження найдовшого слова
  // Порівнюємо довжину поточного слова з довжиною найдовшого слова
 return longestWord; // Інакше повертаємо попереднє найдовше слово без змін
...
function findDuplicateElements(arr) {
  if(!Array.isArray(arr)){return []}// Перевіряємо, чи вхідний параметр є масивом, якщо ні, повертаємо пустий масив
       ...
       let duplicateElements=Array(); let i=0;
       while(i<arr.length){
        let el=arr[i];
        if(arr.includes(el,i+1)){duplicateElements.push(el)};
        i++;
       }
 ...
 return duplicateElements;  // Перевіряємо, чи є індекс поточного елемента відмінним від індексу першого входження елемента, та повертаємо результат
...
function capitalizeWords(arr) {
  if(!Array.isArray(arr)){return []}// Перевіряємо, чи вхідний параметр є масивом, якщо ні, повертаємо пустий масив
  else{ let capitalizedArray=Array();
  capitalizedArray=arr.map(el=>{const splitted = el.split("");
  const first = splitted[0].toUpperCase();
  const rest = [...splitted];
  rest.splice(0, 1);
  let rezult = [first, ...rest].join("");
  return el=rezult; 
...
function calculateTotalPrice(arr) {
  if(!Array.isArray(arr)){return 0}// Перевіряємо, чи вхідний параметр є масивом, якщо ні, повертаємо 0
  else{ let ar=Array();
    for(let i=0;i<arr.length;i++){ar[i]=arr[i].price};
    let totalPrice=ar.reduce((accumulator,currentValue)=>accumulator+currentValue);
...
function findLastNegativeNumberIndex(arr) {
  if(!Array.isArray(arr)){return -1} // Перевіряємо, чи вхідний параметр є масивом, якщо ні, повертаємо -1
  else{const lastIndex=arr.findLastIndex(el=>el<0)// Використовуємо метод findLastIndex() для знаходження індексу останнього входження від'ємного числа
